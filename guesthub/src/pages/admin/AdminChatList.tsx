...
const ChatList: React.FC = () => {
  ...
  const filteredItems = items.filter((it) => {
    const name = it.displayName || "Guest";
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="px-5 pt-6 pb-2 border-b border-stone-100 z-10 bg-white sticky top-0">
                ...
                >
                  <div className="flex justify-between items-start mb-1.5">
                    <div className="flex items-center gap-2 overflow-hidden">
                        {isUnread && (
                            <span className="w-2 h-2 rounded-full bg-rose-500 flex-shrink-0 shadow-sm animate-pulse" />
                        )}
                  ...
                  <div className="flex justify-between items-end">
                      <p className={`text-xs line-clamp-1 leading-relaxed pr-4 ${
                          isUnread ? "text-stone-800 font-medium" : "text-stone-500"
                      }`}>
                        {it.lastMessage}
                      </p>
                      
                      {it.unreadCount > 0 && (
                          <span className="flex-shrink-0 h-4 min-w-[16px] px-1 bg-rose-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                              {it.unreadCount}
                          </span>
                      )}
                  </div>
